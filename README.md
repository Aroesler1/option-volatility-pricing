...
### Statistical significance after the repair

The only Diebold-Mariano and Model Confidence Set table in this README (below,
under "Statistical results, stored before the inner-validation repair") was
generated from the unpurged forecasts and is kept as historical. This table
recomputes the same two tests, with the same functions in `vol_forecasting.py`
(`diebold_mariano` against HAR with a Newey-West lag of h-1, `model_confidence_set`
at 90%) and the same parameters (alpha 0.10, 2000 bootstrap draws, seed 0), on
the corrected per-date forecast paths in
`results/altdata_forecasts_h{1,5,21}_purged.csv`. Source: `results/integrity_dm_mcs.csv`,
regenerated by `python report_tables.py --which integrity_dm_mcs --inject README.md`
and verified against the committed forecast paths by
`python run_integrity_report.py --check`.

<!-- RESULTS:INTEGRITY_DM_MCS -->

#### Horizon 1 day

| model | QLIKE mean | QLIKE median | DM vs HAR | p | MCS p | in 90% MCS |
|---|---|---|---|---|---|---|
| lstm_x | 0.1828 | 0.0724 | -2.0790 | 0.0376 | 1.0000 | yes |
| har_x_lasso | 0.1841 | 0.0815 | -3.0555 | 0.0022 | 0.8685 | yes |
| combination | 0.1867 | 0.0782 | -4.1739 | 0.0000 | 0.8560 | yes |
| hgb | 0.1935 | 0.0881 | -1.4849 | 0.1376 | 0.1815 | yes |
| har_rv_iv | 0.1953 | 0.0797 | -3.2499 | 0.0012 | 0.1430 | yes |
| har | 0.2095 | 0.0836 |  |  | 0.0055 | no |
| shar | 0.2102 | 0.0854 | 0.3673 | 0.7134 | 0.0050 | no |
| lstm | 0.2296 | 0.0821 | 2.5537 | 0.0107 | 0.0195 | no |
| persistence | 0.2783 | 0.1049 | 5.4691 | 0.0000 | 0.0000 | no |

MCS(90%) = {lstm_x, har_x_lasso, combination, hgb, har_rv_iv}

#### Horizon 5 days

| model | QLIKE mean | QLIKE median | DM vs HAR | p | MCS p | in 90% MCS |
|---|---|---|---|---|---|---|
| hgb | 0.1851 | 0.0627 | -1.4411 | 0.1496 | 1.0000 | yes |
| har_x_lasso | 0.1866 | 0.0523 | -2.5245 | 0.0116 | 0.8985 | yes |
| combination | 0.1926 | 0.0548 | -2.6797 | 0.0074 | 0.5290 | yes |
| har_rv_iv | 0.2033 | 0.0569 | -0.6192 | 0.5358 | 0.3980 | yes |
| har | 0.2090 | 0.0595 |  |  | 0.0645 | no |
| shar | 0.2099 | 0.0629 | 1.3060 | 0.1916 | 0.0555 | no |
| lstm_x | 0.2356 | 0.0570 | 1.4887 | 0.1366 | 0.0645 | no |
| lstm | 0.2378 | 0.0593 | 1.7263 | 0.0843 | 0.0645 | no |
| persistence | 0.3530 | 0.0953 | 5.1275 | 0.0000 | 0.0000 | no |

MCS(90%) = {hgb, har_x_lasso, combination, har_rv_iv}

#### Horizon 21 days

| model | QLIKE mean | QLIKE median | DM vs HAR | p | MCS p | in 90% MCS |
|---|---|---|---|---|---|---|
| hgb | 0.2275 | 0.0571 | -0.8977 | 0.3694 | 1.0000 | yes |
| combination | 0.2314 | 0.0824 | -1.2905 | 0.1969 | 0.7750 | yes |
| har | 0.2452 | 0.1023 |  |  | 0.4850 | yes |
| shar | 0.2453 | 0.1042 | 0.3823 | 0.7023 | 0.4850 | yes |
| har_rv_iv | 0.2485 | 0.1041 | 0.7209 | 0.4710 | 0.3560 | yes |
| lstm | 0.2543 | 0.0833 | 0.5103 | 0.6099 | 0.3560 | yes |
| har_x_lasso | 0.2546 | 0.0652 | 0.3671 | 0.7136 | 0.4850 | yes |
| lstm_x | 0.3132 | 0.0686 | 2.0123 | 0.0442 | 0.0065 | no |
| persistence | 0.5811 | 0.1220 | 3.7072 | 0.0002 | 0.0005 | no |

MCS(90%) = {hgb, combination, har, shar, har_rv_iv, lstm, har_x_lasso}

<!-- END:INTEGRITY_DM_MCS -->

**Which models beat HAR at 5% after the repair.** A model "beats HAR" here
means a negative DM statistic (lower QLIKE than HAR) with p < 0.05, not
membership in the 90% MCS, which is a separate, coarser test.

- **1 day:** har_rv_iv, har_x_lasso, lstm_x and combination beat HAR at 5%.
  LSTM-X is one of them: its p-value moves from 0.0795 in the historical
  table to 0.0376 after the repair, crossing the 5% line. This is a change in
  significance from the purge, not a new claim of a bigger effect; LSTM-X's
  QLIKE mean itself only moves from 0.18727 to 0.18280.
- **5 days:** har_x_lasso and combination beat HAR at 5%. LSTM-X does not
  (p = 0.1366, and its QLIKE mean is worse than HAR's, not better).
- **21 days:** no model beats HAR at 5%. LSTM-X does not either; at this
  horizon it is significantly WORSE than HAR (p = 0.0442, positive DM
  statistic), same direction as the historical table.

The MCS membership also shifts in two places worth stating plainly: at 5 days
the corrected 90% MCS drops `har`, `shar` and `lstm_x`, which the historical
MCS had included, leaving `{hgb, har_x_lasso, combination, har_rv_iv}`; at 21
days it adds `lstm`, which the historical MCS had excluded. This table is the
structural horse race only; it does not change the one-feature-at-a-time Holm
results in the marginal tables further below, which this task did not
recompute.
...
## Statistical results, stored before the inner-validation repair (historical)

Every table below is generated from `results/`. Regenerate them with
`python report_tables.py --inject README.md`. Table A's DM and MCS columns
predate the inner-validation purge; the corrected recomputation of that same
test, on the purged forecast paths, is in "Statistical significance after the
repair" above.
